...
- Réponses JSON alignées avec le schéma commun (voir [`../../.claude/skills/crm/SKILL.md`](../../.claude/skills/crm/SKILL.md))
...
Pour brancher concrètement le copilote sur ta couche custom, ouvre une issue sur le repo : selon les volumes et la complexité, on adapte le contrat `skills/crm/SKILL.md` ou on ajoute un client dédié dans `docs/crm/`. Ce squelette est un point de départ, pas un mode plug-and-play complet.
